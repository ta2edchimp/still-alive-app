...
function getLiveDates( name ) {
  return new Promise( ( resolve, reject ) => {
    fetch(
      ...
      .then( ( content ) => {
        const
          birth = /\{\{birth date([^|]{0,}\|){0,}([0-9]{4})\|([0-9]{1,2})\|([0-9]{1,2})/i.exec( content ),
          birthDate = birth ? new Date( birth[ 2 ] + ' ' + birth[ 3 ] + ' ' + birth[ 1 ] ) : null,
          death = /\{\{death date([^|]{0,}\|){0,}([0-9]{4})\|([0-9]{1,2})\|([0-9]{1,2})/i.exec( content ),
          deathDate = death ? new Date( death[ 2 ] + ' ' + death[ 3 ] + ' ' + death[ 1 ] ) : null;
